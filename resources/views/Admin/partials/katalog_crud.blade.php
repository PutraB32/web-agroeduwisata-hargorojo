<div id="panel-katalog" class="crud-panel mt-8 mb-6" style="display: none;">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold text-gray-800">Manajemen Katalog Desa</h2>
        <button onclick="document.getElementById('modal-create-katalog').classList.remove('hidden')" class="bg-green-800 hover:bg-green-900 text-white font-bold py-2 px-4 rounded shadow">
            + Tambah Katalog
        </button>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        @if($katalog->gambar)
                            <img src="{{ asset('images/katalog/' . $katalog->gambar) }}" alt="Gambar" class="h-16 w-16 object-cover rounded border">
                        @else
                            <div class="h-16 w-16 bg-gray-100 text-gray-400 flex items-center justify-center text-xs rounded border">Kosong</div>
                        @endif
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <p class="text-gray-900 whitespace-no-wrap font-bold">{{ $katalog->kategori }}</p>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <p class="text-gray-900 whitespace-no-wrap">{{ $katalog->judul }}</p>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm max-w-xs">
                        <p class="text-gray-900 whitespace-no-wrap text-xs truncate">{{ $katalog->deskripsi }}</p>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm max-w-xs">
                        <p class="text-green-600 whitespace-no-wrap text-xs truncate">{{ $katalog->url ?: '-' }}</p>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center min-w-[150px]">
                        <button onclick="openEditModalKatalog({{ $katalog->id }}, '{{ addslashes($katalog->kategori) }}', '{{ addslashes($katalog->judul) }}', '{{ addslashes($katalog->deskripsi) }}', '{{ addslashes($katalog->url) }}')" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-1 px-3 rounded text-xs mr-2">Edit</button>
                ...
                @if($katalogs->isEmpty())
                <tr>
                    <td colspan="6" class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center text-gray-500">Belum ada data.</td>
                </tr>
                @endif
            </tbody>
        </table>
...
<div id="modal-create-katalog" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" onclick="document.getElementById('modal-create-katalog').classList.add('hidden')"></div>
    ...
    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
      <form action="{{ route('admin.katalog.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Tambah Katalog Desa</h3>
            <div class="form-group mb-4 text-center">
                <label class="block text-gray-700 text-sm font-bold mb-2">Upload Gambar Katalog <span class="text-xs text-gray-500">(Opsional)</span></label>
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">URL Akses</label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text" name="url">
            </div>
            <div class="mb-4">
...
<!-- Modal Edit Katalog -->
<div id="modal-edit-katalog" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" onclick="document.getElementById('modal-edit-katalog').classList.add('hidden')"></div>
    <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
      <form id="form-edit-katalog" action="" method="POST" enctype="multipart/form-data">
          ...
          @method('PUT')
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Edit Katalog Desa</h3>
            <div class="form-group mb-4 text-center">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Kategori</label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="edit-katalog-kategori" type="text" name="kategori" required>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Judul</label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="edit-katalog-judul" type="text" name="judul" required>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">URL Akses</label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="edit-katalog-url" type="text" name="url">
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Deskripsi</label>
                <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="edit-katalog-deskripsi" name="deskripsi" rows="3"></textarea>
            </div>
          </div>
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            <button type="button" onclick="document.getElementById('modal-edit-katalog').classList.add('hidden')" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Batal</button>
          </div>
      </form>
    </div>
  </div>
...
<script>
    function openEditModalKatalog(id, kategori, judul, deskripsi, url) {
        document.getElementById('modal-edit-katalog').classList.remove('hidden');
        document.getElementById('form-edit-katalog').action = '/admin/katalog/' + id;
        document.getElementById('edit-katalog-kategori').value = kategori;
